import { G, player, TILE_SIZE, getNextLaser } from '../../core/globals.js';
 ...
 * @param dt Delta time
 */
export function updateGlitch(boss: any, dt: number) {
    ...
    switch (boss.state) {
        ...
        case 'RECOVER':
            boss.timer += dt;
            if (boss.timer > 1.0) { boss.timer = 0; boss.state = 'IDLE'; }
            break;
            
        case 'TELEGRAPH_LASER':
            boss.vx = 0; boss.timer += dt;
            if (boss.timer > 1.2) {
                boss.timer = 0; boss.state = 'LASER_ATTACK';
                ...
                for (let trail of trails) {
                    ...
                    l.vx = (ldx / ldist) * 450; l.vy = (ldy / ldist) * 450;
        ...
        case 'LASER_ATTACK':
            boss.timer += dt;
            if (boss.timer > 0.5) { boss.timer = 0; boss.state = 'IDLE'; }
            break;
    }
    ...
    if (boss.lastFlipped !== undefined && boss.lastFlipped !== isFlipped) {
        const mirror = (trail: any[], ax: number) => { 
            for (let i = 1; i < trail.length; i++) trail[i].x = 2 * ax - trail[i].x; 
        };
        mirror(boss.hairTrail1, hX1); mirror(boss.hairTrail2, hX2);
        mirror(boss.maneTrail, mX);   mirror(boss.tailTrail, tX);
    }
