 ...
 */
export function updateSepticus(boss: any, dt: number, bDyn: any) {
    if (handleSepticusDeath(boss, dt, bDyn)) return;
    
    // --- AWAKENING TRIGGER ---
    if (!bDyn.triggered) { 
        if(player.x > TILE_SIZE * 12) {
            bDyn.triggered = true; boss.x = player.x - boss.width/2; playSound('powerup');
        } 
        boss.vx = 0; boss.vy = 0; return; 
    }
    ...
    if (player.x < TILE_SIZE * 11) { 
        boss.vx = 0; boss.phase = 0; boss.timer = 0; boss.y += (boss.vy ?? 0) * dt;
        if(boss.y > (boss.startY || 0)) { boss.y = (boss.startY || 0); boss.vy = 0; } else boss.vy = (boss.vy ?? 0) + 800 * dt; 
        return; 
    }
    ...
    if (boss.phase === 0) { // Phase 0: Float and track
        ...
        if(boss.timer > 0.8) { boss.phase = 2; boss.timer = 0; playSound('shoot'); } 
        ...
        let spd2 = 140; boss.vx = (player.x < boss.x + boss.width/2) ? -spd2 : spd2; 
        boss.x += boss.vx * dt; boss.x = Math.max(TILE_SIZE * 10, Math.min(TILE_SIZE * 90, boss.x));
        if(boss.timer > 0.6 && bDyn.throwsLeft > 0) {
            boss.timer = 0; bDyn.throwsLeft--;
    ...
    if (Math.random() < 30 * dt) { 
        let p = getNextParticle(); p.active = true; p.type = 'normal'; p.size = Math.random()*3 + 2; 
        p.x = boss.x + Math.random()*boss.width; p.y = boss.y + boss.height; 
        p.vx = (Math.random()-0.5)*15; p.vy = 40 + Math.random()*80; p.color = '#3ee855'; p.life = 0; p.maxLife = 1.0 + Math.random(); 
    }
    if ((boss.vx ?? 0) !== 0 && Math.random() < 50 * dt) { 
        let p = getNextParticle(); p.active = true; p.type = 'normal'; p.size = Math.random()*4 + 3; 
        p.x = boss.x + ((boss.vx ?? 0) > 0 ? boss.width : 0) + (Math.random()-0.5)*30; p.y = 13 * TILE_SIZE; 
        p.vx = (boss.vx ?? 0) * 0.4 + (Math.random()-0.5)*80; p.vy = -180 - Math.random()*150; 
