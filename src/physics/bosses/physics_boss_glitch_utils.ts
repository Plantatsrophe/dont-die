 ...
 */
export function updateLivingChain(chain: {x:number, y:number}[], targetLength: number, headX: number, headY: number, idealDist: number, waveOffset: number, windX: number = 0, windY: number = 0, wiggle: number = 0.5) {
 ...
 */
export function findGlitchHatPeaks(frame: number[]) {
    ...
    for (let y = 0; y < 32; y++) {
        for (let x = 0; x < 64; x++) {
            if (frame[y * 64 + x] === 10) {
                if (x < 32) {
                    if (y < left.minY) { left.minY = y; left.minX = x; left.maxX = x; }
                    else if (y === left.minY) { left.minX = Math.min(left.minX, x); left.maxX = Math.max(left.maxX, x); }
                } else {
                    if (y < right.minY) { right.minY = y; right.minX = x; right.maxX = x; }
                    else if (y === right.minY) { right.minX = Math.min(right.minX, x); right.maxX = Math.max(right.maxX, x); }
                }
            }
        }
    }
    if (left.minY !== 99) { left.bestX = (left.minX + left.maxX) / 2; left.bestY = left.minY; }
    if (right.minY !== 99) { right.bestX = (right.minX + right.maxX) / 2; right.bestY = right.minY; }
 ...
 */
export function findGlitchBodyAnchors(frame: number[]) {
        ...
        for (let x = 0; x < 64; x++) {
            ...
            if (p === 18 || p === 11 || p === 17) {
                ...
                if (x > 20 && x < 45 && y > 15 && y < 35 && y < neck.minY) {
                    neck.minY = y; neck.x = x; neck.y = y;
                }
            }
        }
    }
    if (minX !== 99) { rear.left = minX; rear.right = maxX; }
    return { neck, rear };
}
