...
export function updateLasers(dt: number) {
    ...
    for (let i = lasers.length - 1; i >= 0; i--) {
        ...
        if (l.reflectionPhase === 'ABSORBING') {
            ...
            if (Math.random() > 0.4) {
                let p = getNextParticle(); p.active = true; p.type = 'normal'; p.size = 2;
                let ang = Math.random() * Math.PI * 2;
                let dist = 15 + Math.random() * 15;
                p.x = l.x + Math.cos(ang) * dist; p.y = l.y + Math.sin(ang) * dist;
                p.vx = -Math.cos(ang) * 150; p.vy = -Math.sin(ang) * 150;
                p.life = 0.15; p.maxLife = 0.15; p.color = '#00ffff';
            }
            ...
            l.beamTimer = (l.beamTimer || 0) - dt;
            if (l.beamTimer <= 0) l.active = false;
        ...
        if (!l.passThroughTiles) {
            for (let t of getCollidingTiles(l)) {
                if (t.type === 1) { hitWall = true; break; }
            }
        }
        ...
        if (!l.reflected) {
            for (let r of reflectorPool) {
                if (r.active && r.isUsable && checkRectCollision(l, r)) {
                    l.vx = 0; l.vy = 0; // Stop the projectile
                    l.reflected = true;
