...
function updateGame(dt: number) {
    ...
    if (boss && boss.active && boss.isSinking) { 
        camTX = boss.x + boss.width / 2; camTY = boss.y + boss.height / 2; 
    }
            ...
            if (!i.collected && checkRectCollision(player, i)) {
                ...
                if (i.type === 'hotdog') { player.lives++; playSound('powerup'); }
                    ...
                    if (boss && boss.active) {
                        ...
                        boss.hp--; boss.hurtTimer = 0.5; G.isMapCached = false; playSound('explosion');
                        if (boss.hp <= 0) bossExplode();
                    }
                }
                else if (i.type === 'detonator') { 
                    playSound('powerup'); 
                    if (boss && boss.active) { bossExplode(); player.cutsceneTimer = 0; G.gameState = 'CREDITS_CUTSCENE'; }
                }
    ...
    for (let i = G.enemies.length - 1; i >= 0; i--) {
        ...
        if (e.type === 'bot') {
            ...
            if (Math.abs(player.y - e.y) < 150 && Math.abs(player.x - e.x) < 500) {
                ...
                if (e.cooldown <= 0) {
                    e.cooldown = 1.6; 
                    let l = getNextLaser(); l.active = true; 
                    l.x = e.dir === 1 ? e.x + e.width : e.x - 16; 
                    l.y = e.y + 4; l.width = 16; l.height = 4; l.vx = 350 * e.dir; 
                    playSound('laser');
                }
            }
        }
    ...
    for (let l of laserPool) {
        ...
        l.y += (l.vy || 0) * dt;
        let hitWall = false;
        for (let t of getCollidingTiles(l)) { if (t.type === 1) hitWall = true; }
        if (hitWall || l.x < 0 || l.x > G.mapCols * TILE_SIZE || l.y < 0 || l.y > G.mapRows * TILE_SIZE) { l.active = false; continue; }
        if (checkRectCollision(player, l)) { playerDeath(); return; }
    }
}
...
export function gameLoop(timestamp: number) {
    ...
    if (dt > 0.1) dt = 0.1; // Prevention of massive jumps in logic if backgrounded
    lastTime = timestamp;
    ...
    if (['PLAYING', 'DYING', 'LEVEL_CLEAR', 'VALVE_CUTSCENE'].includes(G.gameState)) {
        ...
        while (rem > 0) {
            ...
            if (['GAMEOVER', 'WIN'].includes(G.gameState as any)) break;
        }
    } 
