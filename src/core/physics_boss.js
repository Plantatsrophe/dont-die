...
export function updateBoss(dt) {
    ...
    if (boss.type==='masticator') {
        ...
        if (boss.isDying) {
            ...
            if (Math.random() < 30 * dt) { let p = particlePool.find(pp=>!pp.active); if(p){ p.active=true; p.type='normal'; p.size=Math.random()*6+4; p.x=boss.x+Math.random()*boss.width; p.y=boss.y+Math.random()*boss.height; p.vx=(Math.random()-0.5)*400; p.vy=(Math.random()-0.5)*400; p.color=(Math.random()>0.5?'#3ee855':'#ffffff'); p.life=0; p.maxLife=0.6+Math.random()*0.4; } }
            if (boss.timer > 3.0) {
        ...
        if (Math.random() < 30 * dt) { let p = particlePool.find(pp=>!pp.active); if (p) { p.active = true; p.type = 'normal'; p.size = Math.random()*3 + 2; p.x = boss.x + Math.random()*boss.width; p.y = boss.y + boss.height; p.vx = (Math.random()-0.5)*15; p.vy = 40 + Math.random()*80; p.color = '#3ee855'; p.life = 0; p.maxLife = 1.0 + Math.random(); } }
        if (boss.vx !== 0 && Math.random() < 50 * dt) { let p = particlePool.find(pp=>!pp.active); if (p) { p.active = true; p.type = 'normal'; p.size = Math.random()*4 + 3; p.x = boss.x + (boss.vx > 0 ? boss.width : 0) + (Math.random()-0.5)*30; p.y = 13 * TILE_SIZE; p.vx = boss.vx * 0.4 + (Math.random()-0.5)*80; p.vy = -180 - Math.random()*150; p.color = '#3ee855'; p.life = 0; p.maxLife = 0.5 + Math.random()*0.5; } }
    } else if (boss.type==='warden') { if(player.y<boss.y) boss.y-=70*dt; boss.x+=Math.cos(boss.timer*4)*80*dt; }
    else if (boss.type==='core') { if(boss.timer>1.5){boss.timer=0;let l=laserPool.find(lp=>!lp.active);if(l){l.active=true;l.width=16;l.height=8;l.x=boss.x+boss.width/2;l.y=player.y+player.height/2;l.vx=(Math.random()>0.5?-250:250);playSound('shoot');}} }
    else if (boss.type==='goliath') { boss.x=Math.max(boss.x,G.camera.x-30); if(boss.timer>2.0&&G.gameState!=='CREDITS_CUTSCENE'&&G.gameState!=='CREDITS'){boss.timer=0;for(let i=0;i<3;i++){let l=laserPool.find(lp=>!lp.active);if(l){l.active=true;l.width=30;l.height=15;l.x=boss.x+boss.width;l.y=boss.y+40+(i*40);l.vx=400+Math.random()*100;}}playSound('shoot');} }
}
