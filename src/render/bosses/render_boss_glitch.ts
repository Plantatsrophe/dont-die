...
import { drawFiberStrands } from '../utils/render_utils_fiber.js';
import type { IBoss } from '../../types.js';

/**
 * Glitch: Rider on Virtual Steed (64x64).
 */
export function drawGlitch(boss: IBoss) {
 ...
 */
function renderGlitchTelegraph(boss: IBoss) {
    ...
    ctx.rotate(rotSpeed);
    ctx.setLineDash([10, 5]); ctx.lineDashOffset = -time * 0.1;
    ctx.strokeStyle = `hsl(${(time * 0.2) % 360}, 100%, 70%)`;
    ctx.lineWidth = 4 + intensity * 8;
    ctx.beginPath(); ctx.arc(0, 0, 60 + intensity * 20, 0, Math.PI * 2); ctx.stroke();
    
    ctx.setLineDash([]); ctx.strokeStyle = '#ffffff'; ctx.lineWidth = 1; ctx.globalAlpha = intensity * 0.5;
    ctx.beginPath(); ctx.arc(0, 0, 40 + Math.sin(time*0.1)*10, 0, Math.PI * 2); ctx.stroke();
    ctx.restore();
}
