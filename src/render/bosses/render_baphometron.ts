...
import { BaphometronController } from '../../logic/bosses/baphometron.js';
 ...
 */
function renderBaphometronTelegraphs(controller: BaphometronController) {
 ...
 */
function renderBaphometronLimbs(controller: BaphometronController) {
    const limbs = controller.getLimbs();

    limbs.forEach(limb => {
        if (!limb.active || limb.state === 'telegraph') return;
 ...
 */
function renderBaphometronSpikes(controller: BaphometronController) {
    const spikes = controller.getSpikes();

    spikes.forEach(s => {
        if (!s.active || s.state === 'inactive') return;
 ...
 */
function renderBaphometronLaser(controller: BaphometronController) {
    if (!controller.isExecutingLaser) return;
    ...
    // 3.5 -> 2.0: Charge
    // 2.0 -> 0.0: Blast
    if (t > 3.5) return; // Wait for head to arrive before starting laser visuals
 ...
 */
function renderBaphometronHead(controller: BaphometronController) {
    if (!controller.isExecutingLaser) return;
