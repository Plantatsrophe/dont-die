...
export function renderParallax() {
    ...
    if (bId === 0) { // Slums
    ...
    } else if (bId === 1) { // Acid/Sewer
        let px = camera.x * 0.3;
        
        // Faint Brick Pattern (The main wall)
        ...
        for (let r = 0; r < canvas.height / 20 + 2; r++) {
            ...
            for (let c = -1; c < canvas.width / 40 + 2; c++) {
                let bx = (c * 40 + shift - (camera.x * 0.1) % 40);
                ctx.beginPath(); ctx.moveTo(bx, ry); ctx.lineTo(bx, ry + 20); ctx.stroke();
            }
        }

        // Dark Tunnel Openings (Obscure the bricks)
        for (let a = 0; a < 4; a++) {
            ...
            // Subtle internal perspective shadow
        ...
        for(let i = 0; i < 6; i++) {
            ...
            // Organic Moss/Slime (Spatial Cleanup)
