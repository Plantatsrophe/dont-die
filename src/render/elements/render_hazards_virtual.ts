...
export function renderVirtualHazards() {
    ...
    for (let s of corruptedSectors) {
        ...
        if (s.isActive) {
            ...
            ctx.fillStyle = 'rgba(255, 0, 255, 0.2)'; ctx.fillRect(s.x, s.y, s.width, s.height);
        } else {
            ctx.globalAlpha = 0.3;
        }
    ...
    for (let n of malwareNodes) {
        ...
        ctx.strokeStyle = '#ff00ff'; ctx.shadowBlur = 12; ctx.shadowColor = '#ff00ff';
        ctx.globalCompositeOperation = 'lighter';
        ...
        if (n.state === 'EXPANDING') { ctx.fillStyle = 'rgba(255, 0, 255, 0.4)'; ctx.fill(); }
        ctx.restore();
    }
}
