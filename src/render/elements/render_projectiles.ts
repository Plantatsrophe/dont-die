...
export function renderProjectiles() {
    ...
    for (let l of laserPool) {
        ...
        if (l.reflectionPhase === 'ABSORBING') {
            ...
            ctx.globalCompositeOperation = 'lighter'; ctx.shadowBlur = 10; ctx.shadowColor = `hsl(${l.hue}, 100%, 60%)`;
            ctx.fillStyle = '#ffffff'; ctx.fillRect(l.x, l.y, 16, 4);
            ctx.strokeStyle = `hsl(${l.hue}, 100%, 60%)`; ctx.lineWidth = 2; ctx.strokeRect(l.x, l.y, 16, 4);
        } else {
