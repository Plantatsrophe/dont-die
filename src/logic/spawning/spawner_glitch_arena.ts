...
export function buildGlitchArena() {
    if (G.currentLevel !== 79) return;

    reflectorPool.length = 0;
    reflectorPool.push(
        { x: 800, y: 200, width: 40, height: 40, active: true, isUsable: true },
        { x: 2000, y: 100, width: 40, height: 40, active: true, isUsable: true },
        { x: 3200, y: 200, width: 40, height: 40, active: true, isUsable: true }
    );

    let curX = 100, curRow = 7; 
    if (G.map[12]) G.map[12][4] = 1; 
    if (G.map[10]) G.map[10][6] = 1; 
    ...
    for (let target of targets) {
        while (curX < target.x - 60) {
            ...
            if (G.map[curRow] && G.map[curRow][col] !== undefined) G.map[curRow][col] = 1;
        }
        let col = Math.floor(target.x / TILE_SIZE);
        if (G.map[target.row]) {
            for (let i = -1; i <= 2; i++) {
                if (G.map[target.row][col + i] !== undefined) G.map[target.row][col + i] = 1;
            }
        }
        curX = target.x; curRow = target.row;
    }
}
