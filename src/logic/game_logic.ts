...
export function updateGame(dt: number) {
    ...
    if (G.boss && G.boss.active && G.boss.isSinking) { 
        camTX = G.boss.x + G.boss.width / 2; camTY = G.boss.y + G.boss.height / 2; 
    }
        ...
        if (ent.type === 'hotdog' || ent.type === 'checkpoint' || ent.type === 'valve' || ent.type === 'detonator' || ent.type === 'gear') {
            ...
            if (!i.collected && checkRectCollision(player, i)) {
                ...
                if (i.type === 'hotdog') { player.lives++; playSound('powerup'); }
                    ...
                    if (!G.checkpointPos || G.checkpointPos.x !== i.x + 8 || G.checkpointPos.y !== i.y - 2) {
                        G.checkpointPos = { x: i.x + 8, y: i.y - 2 }; playSound('powerup'); 
                        for (let pC = 0; pC < 20; pC++) {
                            let p = getNextParticle(); p.active = true; p.type = 'checkpoint'; 
                            p.x = i.x + 16; p.y = i.y + 16;
                            p.vx = (Math.random() - 0.5) * 100; p.vy = -50 - Math.random() * 100;
                            p.size = 6; p.life = 0.5 + Math.random() * 0.5; p.maxLife = 1.0;
                        }
                    }
                }
                    ...
                    if (G.boss && G.boss.active) {
                        G.gameState = 'VALVE_CUTSCENE'; G.valveCutsceneTimer = 0; 
                        G.activeValvePos = { x: i.x, y: i.y }; G.purifiedValves.push({ x: i.x, y: i.y });
                        G.boss.hp--; G.boss.hurtTimer = 0.5; G.isMapCached = false; playSound('explosion');
                        if (G.boss.hp <= 0) bossExplode();
                    }
                }
                else if (i.type === 'detonator') { 
                    playSound('powerup'); 
                    if (G.boss && G.boss.active) { bossExplode(); player.cutsceneTimer = 0; G.gameState = 'CREDITS_CUTSCENE'; }
                }
            ...
            if (checkRectCollision(player, e)) {
                if (player.vy > 0 && player.y + player.height - player.vy * dt <= e.y + 15) {
                    playSound('stomp'); player.vy = -400; player.doubleJump = true; addScore(200);
                    for (let pC = 0; pC < 20; pC++) {
                        let rad = Math.random() * Math.PI * 2, spd = 50 + Math.random() * 150, p = getNextParticle();
                        p.active = true; p.type = 'gear'; p.x = e.x + e.width / 2; p.y = e.y + e.height / 2;
                        p.vx = Math.cos(rad) * spd; p.vy = Math.sin(rad) * spd - 50;
                        p.size = 16; p.life = 0.8 + Math.random() * 0.4; p.maxLife = 1.2;
                    }
                    const idx = G.enemies.indexOf(e); if (idx !== -1) G.enemies.splice(idx, 1);
    ...
    for (let i = G.enemies.length - 1; i >= 0; i--) {
        ...
        if (e.type === 'bot') {
            ...
            let hitWall = false; for (let t of getCollidingTiles(e)) { if (t.type === 1) hitWall = true; }
            let pitCheck = { x: e.x + (e.dir === 1 ? e.width : -1), y: e.y + e.height + 1, width: 1, height: 1 };
            let overPit = true; for (let t of getCollidingTiles(pitCheck)) { if (t.type === 1 || t.type === 6) overPit = false; }
            ...
            if (Math.abs(player.y - e.y) < 150 && Math.abs(player.x - e.x) < 500) {
                e.cooldown -= dt;
                if (e.cooldown <= 0) {
                    e.cooldown = 1.6; let l = getNextLaser(); l.active = true; 
                    l.x = e.dir === 1 ? e.x + e.width : e.x - 16; l.y = e.y + 4; 
                    l.width = 16; l.height = 4; l.vx = 350 * e.dir; playSound('laser');
                }
            }
        }
    }
