...
export function updatePhysics(dt) {
    ...
    if (G.gameState === 'DYING') {
        if (player.dyingTimer === undefined)
            player.dyingTimer = 0;
    ...
    if (G.gameState === 'VALVE_CUTSCENE') {
        ...
        if (G.valveCutsceneTimer > 5.0) {
            G.gameState = 'PLAYING';
            G.activeValvePos = null;
            if (G.boss)
                G.boss.vibrateX = 0;
        }
        ...
        if (G.boss && G.boss.active)
            G.boss.vibrateX = Math.sin(Date.now() * 0.05) * 8;
        return;
    }
    ...
    for (let t of clashing) {
        ...
        if (t.type === 3 && checkRectCollision(player, { x: t.rect.x + 8, y: t.rect.y + 20, width: 24, height: 20 }))
            hitSpike = true;
        if (t.type === 15)
            hitSpike = true; // Alien Acid spikes
        if (t.type === 5) {
            hitGoal = true;
    ...
    if (player.isClimbing) {
        ...
        if (player.vy > 800)
            player.vy = 800; // Terminal velocity
    }
    ...
    if (player.y > G.mapRows * TILE_SIZE)
        playerDeath(); // Fall off bottom of map
    if (player.x < 0)
        player.x = 0; // Hard clamp left map border
    if (player.x + player.width > G.mapCols * TILE_SIZE)
        player.x = G.mapCols * TILE_SIZE - player.width; // Hard clamp right border
