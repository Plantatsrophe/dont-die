...
export class BaphometronController {
    ...
    isExecutingLaser = false; // Blocks standard attacks during scripted events
    hasFinished = false; // Clamps logic once time hits zero
    globalCooldown = 0; // Prevents simultaneous starts (0.5s)
    // Attack timers for recurring moves
    fistCooldown = 0;
    kickCooldown = 0;
    laserCooldown = 25.0; // Initial laser timer
    // Scripted event flags to ensure they only fire once
    laser30Triggered = false;
    scriptedEventTimer = 0; // Duration for pauses during transitions
     ...
     */
    constructor(onComplete, onShake) {
        ...
        [...this.fistPool, ...this.kickPool].forEach(limb => {
            ...
            // --- EXECUTING PHASE ---
                ...
                if (limb.type === 'fist') {
                    ...
                    if (limb.startSide === 'left' && limb.x > worldWidth)
                        limb.active = false;
                    if (limb.startSide === 'right' && limb.x + limb.width < 0)
                        limb.active = false;
                }
            }
            // --- RECOVERY PHASE ---
                ...
                limb.timer -= dt;
                if (limb.timer <= 0)
                    limb.active = false; // Final despawn
            }
            // --- LAWNMOWER EFFECT ---
    ...
    handleFinalLaser() {
        ...
        if (this.onSurvivalComplete)
            this.onSurvivalComplete();
    }
    handleRecurringAttacks(dt) {
    ...
    triggerLaser(context, duration = 5.0) {
