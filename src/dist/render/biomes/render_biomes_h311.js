import { G, TILE_SIZE, canvas, ctx } from '../../core/globals.js';
/**
 ...
 */
function drawLandmark(ctx, cX, seed, typeIdx, color) {
    ...
    if (typeIdx === 0) {
        ...
        ctx.beginPath();
        const tW = sW * 0.7;
        ctx.moveTo((sW - tW) / 2, 0);
        ctx.lineTo((sW + tW) / 2, 0);
        ...
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(sW * 0.1, 20);
        ctx.bezierCurveTo(-60 - seed * 50, -20, -80, -120 - seed * 60, -30, -180 - seed * 100);
        ctx.bezierCurveTo(-40, -100, -20, -50, sW * 0.25, 10);
        ctx.closePath();
        ctx.moveTo(sW * 0.9, 20);
        ctx.bezierCurveTo(sW + 60 + seed * 50, -20, sW + 80, -120 - seed * 60, sW + 30, -180 - seed * 100);
        ctx.bezierCurveTo(sW + 40, -100, sW + 20, -50, sW * 0.75, 10);
        ctx.closePath();
        ctx.fill();
        ctx.beginPath();
        ctx.rect(sW * 0.15, sH, sW * 0.7, 50);
        ctx.fill();
        ctx.fillStyle = skyColor;
        ctx.shadowBlur = 0;
        ctx.beginPath(); // Face
        ctx.moveTo(sW * 0.2, sH * 0.4);
        ctx.lineTo(sW * 0.2 + 25, sH * 0.4 + 15);
        ctx.lineTo(sW * 0.2 + 25, sH * 0.4);
        ctx.closePath();
        ctx.moveTo(sW * 0.8, sH * 0.4);
        ctx.lineTo(sW * 0.8 - 25, sH * 0.4 + 15);
        ctx.lineTo(sW * 0.8 - 25, sH * 0.4);
        ctx.closePath();
        ctx.fill();
        for (let j = 0; j < 3; j++)
            ctx.fillRect(sW * 0.25, sH + 10 + j * 12, sW * 0.5, 5);
        ctx.restore();
        ...
        ctx.beginPath();
        ctx.arc(chainX, chainEndY, 18, 0, Math.PI * 2);
        ctx.fillStyle = color;
        ctx.fill();
    }
    else if (typeIdx === 1) {
        ...
        for (let k = 1; k < 6; k++) {
            ...
            if (gapY < canvas.height - 30)
                ctx.fillRect(cX + obW * 0.2, gapY, obW * 0.6, 4);
        }
        ctx.restore();
    }
    else {
        // 3. DRAW INDUSTRIAL SPIRE (Capped at 400px to stay below moon midline)
        const spW = 30 + seed * 20;
        const spH = 250 + seed * 150;
        const spY = canvas.height - spH;
        ctx.save();
        ctx.fillStyle = color;
        ctx.beginPath();
        ctx.rect(cX, spY + spH * 0.1, spW, spH * 0.9);
        ctx.moveTo(cX, spY + spH * 0.1);
        ctx.lineTo(cX + spW / 2, spY);
        ctx.lineTo(cX + spW, spY + spH * 0.1);
        ctx.fill();
        ctx.fillStyle = skyColor;
        ctx.shadowBlur = 0;
        for (let k = 1; k < 12; k++) {
            const gY = spY + (spH * 0.08 * k);
            if (gY < canvas.height - 40)
                ctx.fillRect(cX + spW * 0.1, gY, spW * 0.8, 4);
        }
        ctx.restore();
    }
    ctx.restore();
...
function drawVolcano(ctx, px) {
    ...
    const drawRiver = (startX, startY, segments) => {
        ctx.lineJoin = 'round';
        ctx.lineCap = 'round';
