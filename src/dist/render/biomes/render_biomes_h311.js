...
export function drawH311Midground(px) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        if (i % 2 === 0) {
            ...
            ctx.beginPath();
            const topW = sW * 0.7;
            ctx.moveTo((sW - topW) / 2, 0);
            ctx.lineTo((sW + topW) / 2, 0);
            ...
            ctx.lineTo(sW * 0.2 + eyeW, sH * 0.4 + eyeH);
            ctx.lineTo(sW * 0.2 + eyeW, sH * 0.4);
            ctx.closePath();
            ctx.moveTo(sW * 0.8, sH * 0.4);
            ctx.lineTo(sW * 0.8 - eyeW, sH * 0.4 + eyeH);
            ctx.lineTo(sW * 0.8 - eyeW, sH * 0.4);
            ctx.closePath();
            ctx.fill();
            for (let j = 0; j < 3; j++)
            ...
            ctx.beginPath();
            ctx.moveTo(chainX, 0);
            ctx.bezierCurveTo(chainX + 15, 60, chainX - 15, 160, chainX, 250 + seed * 150);
            ctx.lineWidth = 25;
            ctx.strokeStyle = drawColor;
            ctx.stroke();
            ctx.beginPath();
            ctx.arc(chainX, 250 + seed * 150, 18, 0, Math.PI * 2);
            ctx.fillStyle = drawColor;
            ctx.fill();
        }
        else {
            ...
            ctx.lineTo(cX + obW * 0.2, obY + obH * 0.1);
            ctx.lineTo(cX + obW * 0.8, obY + obH * 0.1);
            ctx.lineTo(cX + obW, canvas.height);
            ctx.moveTo(cX + obW * 0.2, obY + obH * 0.1);
            ctx.lineTo(cX + obW * 0.5, obY);
            ctx.lineTo(cX + obW * 0.8, obY + obH * 0.1);
            ctx.fill();
            // Mechanical Details
            ctx.fillStyle = skyColor;
